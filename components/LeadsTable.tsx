...
const STATUS_CONFIG: Record<string, { bg: string; color: string; dot: string }> = {
  ...
  contacted: { bg: '#FFF7ED', color: '#C2410C', dot: '#F97316' },
  ...
  on_hold:   { bg: '#FFFBEB', color: '#92400E', dot: '#F59E0B' },
  nurturing: { bg: '#F0FDFA', color: '#0F766E', dot: '#14B8A6' },
};
...
function MobileLeadCard({ lead, isNew, headers, statusLabelMap, assignment, onEditClick, onHistoryClick, onViewClick }: MobileCardProps) {
  const hasHeader = (name: string) => headers.includes(name);
  const branchLabel = hasHeader('Selected Branch') ? 'Selected Branch' : 'Address';
  const assigneeName = assignment?.assignee_name ?? assignment?.assignee_email ?? lead.assignedRepName ?? null;
    ...
    <div className={[
      ...
      <div className="px-4 pt-4 pb-3 flex items-start justify-between gap-3">
        <div className="flex flex-col gap-0.5 min-w-0">
          <span className="text-base font-bold text-[#0F172A] truncate">{lead.fullName || '—'}</span>
          <a href={`tel:${lead.phoneNumber}`} className="text-sm font-medium text-[#0A6BA8] flex items-center gap-1">
...
function buildDynamicColumns(
  ...
  return headers.map((header, colIndex): ColDef<Lead> => {
    ...
    switch (header) {
      ...
      case 'Date':        return { ...base, width: 110 };
      case 'Lead Source': return { ...base, width: 120 };
      case 'Name':     return { ...base, flex: 1, minWidth: 130, width: undefined };
      case 'Phone Number': return { ...base, width: 140, sortable: false };
      case 'Email Address':
      case 'Email':    return { ...base, flex: 1, minWidth: 180, width: undefined };
      case 'Location':
      case 'Address':
      case 'Selected Branch': return { ...base, flex: 1, minWidth: 140, width: undefined };
      case 'Campaign':
      case 'Campaign Name':   return { ...base, width: 140 };
      default:         return base;
    }
  });
}
...
function LeadEditModal({
  ...
  failReasons, candidates, actor, currentAssignment,
  onUpdate, onAssignmentChanged, onClose,
}: EditModalProps) {
  ...
  const validate = (): boolean => {
    ...
    if (showRejection) {
      if (!failReasonId)      errs.failReason = 'Select a reason';
      if (!rejNotes.trim())   errs.rejNotes   = 'Notes are required';
    }
    setErrors(errs);
    ...
    try {
      if (statusChanged) {
        await onUpdate({
          ...
          ...(showRejection && failReasonId !== '' ? { failReasonId: Number(failReasonId) } : {}),
      ...
      const shouldPatchAssignee = autoAssign || (assigneeChanged && lead.leadId);
      ...
      if (shouldPatchAssignee && lead.leadId) {
        ...
        if (!res.ok) {
          ...
          throw new Error((body as Record<string, unknown>).error as string ?? 'Failed to update assignee');
        }
      }
      onAssignmentChanged();
      onClose();
  ...
  const currentAssigneeName = (() => {
    ...
    return currentAssignment?.assignee_name ?? currentAssignment?.assignee_email ?? null;
  })();
      ...
      <div className="my-auto w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-start justify-between border-b border-[#F1F5F9] px-6 py-4">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5">
              ...
              {lead.phoneNumber && (
                <a href={`tel:${lead.phoneNumber}`} className="flex items-center gap-1 text-xs text-[#0b6cbf] hover:underline">
        ...
        <div className="flex flex-col divide-y divide-[#F1F5F9]">
          ...
          <div className="flex flex-col gap-4 px-6 py-5">
            ...
            {showFollowUp && (
              <div className="rounded-xl border border-[#BFDBFE] bg-[#EFF6FF] px-4 py-4 flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    value={fuAssigneeId}
                    onChange={(e) => { setFuAssigneeId(e.target.value); setErrors(p => ({ ...p, fuAssignee: '' })); }}
            ...
            {/* ── Rejection panel (appears when status is_rejection) ── */}
            {showRejection && (
              <div className="rounded-xl border border-[#FCA5A5] bg-[#FEF2F2] px-4 py-4 flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    value={failReasonId}
                    onChange={(e) => { setFailReasonId(Number(e.target.value) || ''); setErrors(p => ({ ...p, failReason: '' })); }}
                    className={`w-full rounded-lg border px-3 py-2 text-sm text-[#0F172A] focus:outline-none focus:ring-2 ${errors.failReason ? 'border-red-400 focus:ring-red-200' : 'border-[#E2E8F0] focus:border-[#0b6cbf] focus:ring-[#0b6cbf]/20'}`}
                  >
                  ...
                  {errors.failReason && <p className="text-xs text-red-500">{errors.failReason}</p>}
                </div>
...
function LeadViewModal({ lead, statusLabelMap, currentAssignment, onClose }: ViewModalProps) {
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') onClose(); };
    document.addEventListener('keydown', onKey);
    document.body.style.overflow = 'hidden';
  ...
  const assigneeName = currentAssignment?.assignee_name ?? currentAssignment?.assignee_email ?? lead.assignedRepName ?? null;
  ...
  const extraFields = [
    ...
    { label: 'Membership Interest', value: lead.membershipInterest, full: false },
...
export default function LeadsTable({
  leads, loading, statusFilter = 'all',
  ...
  const filtered = useMemo(() => {
    if (statusFilter === 'followUp')   return leads.filter(l => followUpSet.has(l.Status ?? ''));
    if (statusFilter === 'unassigned') return leads.filter(l => !l.assignedUserId);
  ...
  const assigneeCellRenderer = useCallback((params: ICellRendererParams<Lead>) => {
    const lead = params.data;
    if (!lead) return null;
    const ctx = params.context as { assignments: Record<number, AssignmentView> };
    const row  = ctx.assignments[lead.rowIndex];
    const name = row?.assignee_name ?? row?.assignee_email ?? lead.assignedRepName ?? null;
    return <span style={{ color: name ? '#0F172A' : '#94A3B8', fontStyle: name ? 'normal' : 'italic' }}>{name ?? 'Unassigned'}</span>;
  }, []);

  // Actions column renderer — icon-only buttons with tooltips; text shown on mobile via MobileLeadCard
  const actionsCellRenderer = useCallback((params: ICellRendererParams<Lead>) => {
    const lead = params.data;
    if (!lead) return null;
    const ctx = params.context as { onEdit: (l: Lead) => void; onHistory: (l: Lead) => void; onView: (l: Lead) => void };
    return (
      <div className="flex items-center gap-1.5">
        <button
          type="button"
  ...
  const columnDefs = useMemo(() => {
    ...
    const assignee: ColDef<Lead> = {
      ...
      valueGetter: (params) => {
        ...
        const ctx = params.context as { assignments: Record<number, AssignmentView> };
        const row = ctx.assignments[lead.rowIndex];
        return row?.assignee_name ?? row?.assignee_email ?? lead.assignedRepName ?? 'Unassigned';
      },
  ...
  const gridContext = useMemo(
    () => ({ actor, assignments, onEdit: setEditingLead, onHistory: setHistoryLead, onView: setViewingLead }),
    [actor, assignments],
  );

  useEffect(() => {
    gridRef.current?.api?.refreshCells({ force: true, columns: ['__assignee'] });
  }, [assignments]);
    ...
    <>
      ...
      <div className="ag-theme-alpine" style={{ flex: '1 1 0', minHeight: 0, width: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <AgGridReact<Lead>
          ...
          getRowId={(params) => String(params.data.leadId ?? params.data.rowIndex)}
