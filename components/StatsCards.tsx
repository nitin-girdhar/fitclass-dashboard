...
// ── main export ───────────────────────────────────────────────────────────────
// Sales tier (rank < 2): follow-up count scoped to own leads only.
// Managers and above: all leads in their visible set.

interface Props {
  stats: StatsData;
  leads: Lead[];
  /** DB-sourced list of status names with requires_followup = true. */
  requiresFollowupStatuses: string[];
  actor: SessionUser;
  activeFilter: CardFilter;
  onFilterChange: (f: CardFilter) => void;
  /** When true, hides the Unassigned card (e.g. on My Leads where it's always 0). */
...
export default function StatsCards({ stats, leads, requiresFollowupStatuses, actor, activeFilter, onFilterChange, hideUnassigned }: Props) {
  ...
  const counts = useMemo(() => {
    ...
    return {
      ...
      // Manager+: all visible leads needing follow-up
      followUp: leads.filter(l =>
        followUpSet.has(l.Status ?? '') &&
        (!isSalesTier || (l as any).assignedUserId === actor.id)
      ).length,
      unassigned: leads.filter(l => !(l as any).assignedUserId).length,
    };
